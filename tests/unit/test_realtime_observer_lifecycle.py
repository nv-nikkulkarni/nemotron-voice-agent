...
class ObserverAudioLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_audio_response_waits_for_every_tts_context_and_final_playout(self) -> None:
        ...
        controller.output_audio_delta_event("AA==", sample_count=2_400, sample_rate=24_000)
    ...
    async def test_audio_response_without_wire_audio_is_a_provider_failure(self) -> None:
        recorder = _Recorder()
        controller = _controller()
        observer = RealtimeLifecycleObserver(
            emit=recorder.emit,
        ...
        start = _owned_start(controller, "empty-audio-run")
        await _push(observer, start, source=self.llm, destination=self.destination)
        await _push(observer, start, source=self.tts, destination=self.destination)
        await _push(observer, TTSStartedFrame(context_id="empty-audio"), source=self.tts, destination=self.destination)
        await _push(observer, LLMTextFrame(text="Hello."), source=self.llm, destination=self.destination)
        await _push(
            observer,
            TTSTextFrame(text="Hello.", raw_text="Hello.", aggregated_by="sentence", context_id="empty-audio"),
            source=self.output,
            destination=self.destination,
        )
        end = LLMFullResponseEndFrame()
        ...
        await _push(
            observer,
            TTSStoppedFrame(context_id="empty-audio"),
            source=self.output,
            destination=self.destination,
        )

        error = next(event for event in recorder.events if event["type"] == "error")
        self.assertEqual(error["error"]["code"], "tts_provider_error")
        done = next(event for event in recorder.events if event["type"] == "response.done")
        self.assertEqual(done["response"]["status"], "failed")
        self.assertEqual(done["response"]["status_details"]["error"]["code"], "tts_provider_error")
        observer.shutdown()
