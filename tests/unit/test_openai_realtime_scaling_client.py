...
def _automatic_audio_item_events(
    *,
    item_id: str,
    ...
    return [
        {"type": "input_audio_buffer.speech_started", "item_id": item_id, "audio_start_ms": audio_start_ms},
        {"type": "input_audio_buffer.speech_stopped", "item_id": item_id, "audio_end_ms": audio_end_ms},
        {"type": "input_audio_buffer.committed", "item_id": item_id, "previous_item_id": previous_item_id},
        {"type": "conversation.item.added", "previous_item_id": previous_item_id, "item": added},
        {"type": "conversation.item.done", "item": done},
    ]
    ...
    async def test_text_input_sends_item_then_response_create_for_both_outputs(self) -> None:
        ...
        for output_modality in ("text", "audio"):
            with self.subTest(output_modality=output_modality):
                ...
                self.assertEqual(item["id"], trace.text_item_id)
                self.assertEqual(trace.submission_order, [item["id"]])
                self.assertTrue(trace.items[item["id"]].submitted)
...
class RealtimeAudioInputLifecycleTests(unittest.IsolatedAsyncioTestCase):
    """Validate every server-defined audio item within one benchmark turn."""

    async def test_split_automatic_wav_is_rejected_after_full_streaming(self) -> None:
        """Reject a multi-item WAV instead of reporting ambiguous latency."""
        audio_path = Path("input.wav")
        client = _realtime_client(input_mode="audio", audio_files=[audio_path], text_inputs=[])
        client._conversation_id = "conv_audio"
        client._collecting_metrics = True
        streamed_audio: list[bytes] = []
        ...
        async def stream(
            ...
            streamed_audio.append(pcm)
            if len(streamed_audio) == 1:
                client._input_audio_bytes = REALTIME_PCM_RATE
                first_item = _automatic_audio_item_events(
                    item_id="item_audio_a",
                    previous_item_id=None,
                    audio_start_ms=100,
                    audio_end_ms=700,
                ...
                events = _received_events(
                    [
                        ...
                        *_text_response_events(
                            ...
                            response_id="resp_audio_a",
                            item_id="item_assistant_a",
                            previous_item_id="item_audio_a",
                            text="First answer",
                        ),
                        {
                            "type": "input_audio_buffer.speech_started",
                            "item_id": "item_audio_b",
                            "audio_start_ms": 700,
                        },
                    ]
                )
                prefetched.extend(events)
                return events[1].received_at, True

            self.assertEqual(pcm, b"\x00\x00" * (REALTIME_PCM_RATE * client.vad_silence_ms // 1000))
            second_item = _automatic_audio_item_events(
                item_id="item_audio_b",
                previous_item_id="item_audio_a",
                audio_start_ms=700,
                audio_end_ms=1200,
            ...
            prefetched.extend(events)
            return events[0].received_at, True

        client._read_wav_as_pcm_24khz = lambda _path: b"\x00\x00"
        client._stream_pcm_with_event_prefetch = stream

        with self.assertRaisesRegex(RealtimeClientError, "automatic VAD split one benchmark WAV"):
            await client._continuous_turn_loop(_RecordingWebSocket())

        self.assertEqual(streamed_audio[0], b"\x00\x00")
        self.assertEqual(streamed_audio[1], b"\x00\x00" * (REALTIME_PCM_RATE * client.vad_silence_ms // 1000))
        self.assertEqual(client.failed_turns, 1)
        self.assertEqual(client.latency_values, [])
        self.assertEqual(client.valid_latency_values, [])
        self.assertEqual(client.response_transcripts, [])
        self.assertEqual(client.response_status_counts, {})
        self.assertTrue(all(not samples for samples in client.server_metric_samples.values()))

    async def test_audio_lifecycle_rejects_invalid_item_order_without_weakening_manual_mode(self) -> None:
        """Keep per-item ordering strict and manual commits singular."""
        automatic = _realtime_client(input_mode="audio", audio_files=[Path("input.wav")], text_inputs=[])
        trace = _TurnInputTrace()
        automatic._capture_input_lifecycle_event(
            {"type": "input_audio_buffer.speech_started", "item_id": "item_a", "audio_start_ms": 100},
            trace,
        )
        with self.assertRaisesRegex(RealtimeClientError, "preceding speech boundary"):
            automatic._capture_input_lifecycle_event(
                {"type": "input_audio_buffer.speech_started", "item_id": "item_b", "audio_start_ms": 200},
                trace,
            )
        with self.assertRaisesRegex(RealtimeClientError, "must follow speech_started/stopped"):
            automatic._capture_input_lifecycle_event(
                {"type": "input_audio_buffer.committed", "item_id": "item_a", "previous_item_id": None},
                trace,
            )

        lagged = _TurnInputTrace(audio_source_end_ms=1_200)
        automatic._capture_input_lifecycle_event(
            {"type": "input_audio_buffer.speech_started", "item_id": "item_lagged", "audio_start_ms": 100},
            lagged,
        )
        with self.assertRaisesRegex(RealtimeTurnError, "before the WAV finished"):
            automatic._capture_input_lifecycle_event(
                {"type": "input_audio_buffer.speech_stopped", "item_id": "item_lagged", "audio_end_ms": 700},
                lagged,
            )

        manual = _realtime_client(
            input_mode="audio",
            turn_mode="manual",
            audio_files=[Path("input.wav")],
            text_inputs=[],
        )
        manual_trace = _TurnInputTrace()
        manual._capture_input_lifecycle_event(
            {"type": "input_audio_buffer.committed", "item_id": "item_manual", "previous_item_id": None},
            manual_trace,
        )
        with self.assertRaisesRegex(RealtimeClientError, "more than one.*manual turn"):
            manual._capture_input_lifecycle_event(
                {"type": "input_audio_buffer.committed", "item_id": "item_extra", "previous_item_id": None},
                manual_trace,
            )
