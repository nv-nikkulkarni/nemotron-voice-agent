...
class RealtimeCapabilityTests(unittest.TestCase):
    def test_static_digest_normalizes_comments_wraps_and_first_sentence(self) -> None:
        ...
        self.assertIn("- weather: check current weather.", rendered)
        self.assertIn("- find_order: Find an order by email or id.", rendered)
    ...
    def test_static_digest_caps_without_splitting_the_last_word(self) -> None:
        ...
        summary = next(line for line in rendered.splitlines() if line.startswith("- long_tool:"))
        self.assertLessEqual(len(summary.removeprefix("- long_tool: ")), 49)
...
class ModelCapabilityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_model_digest_is_validated_rendered_and_cached(self) -> None:
        client = {
            "name": "find_order",
        ...
        generated = {
            ...
            "groups": [{"label": "Orders", "tools": ["find_order"]}],
        }
        ...
        self.assertIn("- find_order: Find an order by email.", first)
        ...
        llm = SimpleNamespace(
            run_structured_inference=AsyncMock(
                return_value={
                    ...
                    "scope_in": [],
                    "scope_out": [],
                    "groups": [{"label": "Unsafe", "tools": ["invented_tool"]}],
                }
            )
        )
