...
_GENERIC_FBA_MODEL = "nvidia/nemotron-realtime-generic-frontend-backend"
...
class RealtimeModelProfileTests(unittest.TestCase):
    ...
    def test_checked_in_profiles_have_one_default_per_pipeline(self) -> None:
        ...
        exact_model_pipelines = {"generic-frontend-backend-agent"}
        self.assertEqual(set(defaults), set(examples_registry.EXAMPLES) - exact_model_pipelines)
        self.assertTrue(all(len(models) == 1 for models in defaults.values()))
        self.assertFalse(profiles[_GENERIC_FBA_MODEL]["default"])
    ...
    def test_checked_in_service_selectors_are_explicit_or_use_registry_defaults(self) -> None:
        ...
        multilingual = profiles["nvidia/nemotron-realtime-multilingual"]
    ...
    def test_parser_allows_explicit_only_pipeline_and_rejects_duplicate_defaults(self) -> None:
        no_default = self._registry()
        no_default["realtime_models"][_GENERIC_MODEL]["default"] = False
        self._load(no_default)

        duplicate = self._registry()
        duplicate["realtime_models"][_CLIENT_TOOLS_MODEL]["default"] = True
        with self.assertRaisesRegex(RuntimeError, "at most one default model"):
