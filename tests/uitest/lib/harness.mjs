...
export function newSignals() {
  return { consoleErrors: [], failedRequests: [], badResponses: [], wsClosures: [] };
}
...
export function attachSignals(page, sig) {
  ...
  // Playwright's ws 'close' event carries no close code, so a clean end-of-session
  // close is indistinguishable from an abnormal one — only 'socketerror' signals a
  // genuine transport fault.
  page.on("websocket", (ws) => ws.on("socketerror", (err) => sig.wsClosures.push(`ws socketerror: ${String(err).slice(0, 100)}`)));
}
...
export async function selectExample(page, { example = "generic", model = "lightning", tts, tools, reasoning, consent = false } = {}) {
  ...
  if (consent) {
    ...
    if (await cb.count()) await cb.first().evaluate((el) => { if (!el.checked) el.click(); }).catch(() => {});
  }
  await sleep(750);
  if (typeof reasoning === "boolean") {
    const cb = popup.locator(".reasoning-toggle input[type=checkbox]");
...
export async function captureBot(page, name, { maxMs = 25000, quietMs = 1700, monitor = "spk_sink.monitor" } = {}) {
  ...
  return { out, sawOnset, responseMs: onsetAt ? onsetAt - t0 : null };
}

// One full spoken turn: wait to be listening, speak, capture the bot, ASR it, read DOM.
export async function turn(page, text, name, { voice, transcribeBot = true, echoToSpk = false, micDevice, monitor } = {}) {
  await waitListening(page);
  await page.evaluate(() => window.__botReset());
  const before = (await readMessages(page)).length;
  const t0 = Date.now();
  await speak(text, `${name}_user`, { voice, echoToSpk, ...(micDevice ? { micDevice } : {}) });
  const cap = await captureBot(page, `${name}_bot`, monitor ? { monitor } : {});
  ...
  return { user: text, botSpoke: cap.sawOnset, botAsr, domUser, domBot, responseMs: cap.responseMs,
