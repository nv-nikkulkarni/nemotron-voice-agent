...
async function landingChecks(browser) {
  ...
  try {
    ...
    const tourTrigger = page.getByRole("button", { name: /open guided introduction/i });
    const hint = page.locator(".tour-hint");
    await tourTrigger.waitFor({ state: "visible", timeout: 3000 }).catch(() => {});
    rec("landing/no-tour-invitation", !(await invitation.isVisible().catch(() => false)));
    rec("landing/passive-tour-hint", await hint.isVisible().catch(() => false));
    rec("landing/no-automatic-animation", !(await tour.isVisible().catch(() => false)));
    rec("landing/tour-trigger-visible", await tourTrigger.isVisible().catch(() => false));
    await tourTrigger.click();
    await tour.waitFor({ state: "visible", timeout: 3000 }).catch(() => {});
    rec("landing/tour-starts-after-click", await tour.isVisible().catch(() => false));
...
async function lifecycleChecks(browser) {
  ...
  try {
    ...
    const tourTrigger = page.getByRole("button", { name: /open guided introduction/i });
    rec("lifecycle/no-tour-invitation", !(await liveInvitation.isVisible().catch(() => false)));
    rec("lifecycle/no-tour-animation", !(await liveTour.isVisible().catch(() => false)));
    rec("lifecycle/no-tour-control", !(await tourTrigger.isVisible().catch(() => false)));
    const timer = await page.locator(".demo-timer").innerText().catch(() => "");
    rec("lifecycle/ten-minute-timer", /10:00|09:59/.test(timer), timer.replace(/\n/g, " "));
    const latencyLabel = await page.locator(".conv-latency__label").innerText().catch(() => "");
    rec("lifecycle/time-to-first-audio-label", /time to first audio/i.test(latencyLabel), latencyLabel);
