...
export async function synthSpeech(text, outWav, { voice = TTS_VOICE, instructions, engine = "remote" } = {}) {
  if (engine === "espeak-ng") {
    const raw = `${outWav}.espeak.wav`;
    await execFileP("espeak-ng", ["-v", "en-us", "-s", "150", "-w", raw, String(text)]);
    await to16kMonoWav(raw, outWav);
    await fs.unlink(raw).catch(() => {});
    return { outWav, durationSec: await wavDuration(outWav) };
  }
  if (engine !== "remote") {
    throw new Error(`Unsupported SQA speech engine: ${engine}`);
  }
