...
_INPUT_SPEECH_BOUNDARY_EVENTS = frozenset(
    {
        "input_audio_buffer.speech_stopped",
        "input_audio_buffer.timeout_triggered",
    }
)
_INPUT_LIFECYCLE_EVENTS = frozenset(
    {
        "input_audio_buffer.speech_started",
        *_INPUT_SPEECH_BOUNDARY_EVENTS,
...
@dataclass(slots=True)
class _TurnInputItemTrace:
    is_audio: bool
    speech_start_ms: int | None = None
    speech_end_ms: int | None = None
    speech_boundary_event: str | None = None
...
class _TurnInputTrace:
    items: dict[str, _TurnInputItemTrace] = field(default_factory=dict)
    submission_order: list[str] = field(default_factory=list)
    response_item_ids: dict[str, tuple[str, ...]] = field(default_factory=dict)
...
class OpenAIRealtimePerfClient:
    ...
    async def _stream_pcm_with_event_prefetch(
        ...
        """Stream complete PCM while preserving concurrently received events."""
        ...
        speech_boundary_at: float | None = None
        try:
            while True:
                ...
                if event_task in done:
                    ...
                    event_type = received.event["type"]
                    if event_type in _INPUT_SPEECH_BOUNDARY_EVENTS:
                        speech_boundary_at = received.received_at
                    if event_type in {"error", "input_audio_buffer.cleared"}:
                        stop_event.set()
                ...
                if stream_task in done:
                    ...
                    if event_task is not None:
                        ...
                        if raced_event is not None:
                            ...
                            raced_event_type = raced_event.event["type"]
                            if raced_event_type in _INPUT_SPEECH_BOUNDARY_EVENTS:
                                speech_boundary_at = raced_event.received_at
                    return (
                        speech_boundary_at or stream_ended_at,
                        speech_boundary_at is not None,
                    )
    ...
    @staticmethod
    def _pending_automatic_input_items(prefetched: deque[_ReceivedEvent]) -> set[str]:
        started: set[str] = set()
        closed: set[str] = set()
        committed: set[str] = set()
        for received in prefetched:
            event = received.event
            ...
            if event_type == "input_audio_buffer.speech_started":
                started.add(item_id)
            elif event_type in _INPUT_SPEECH_BOUNDARY_EVENTS:
                closed.add(item_id)
            elif event_type == "input_audio_buffer.committed":
                committed.add(item_id)
        return {item_id for item_id in started if item_id not in closed or item_id not in committed}

    async def _prefetch_until_automatic_input_items_are_closed(
        self,
        prefetched: deque[_ReceivedEvent],
    ) -> None:
        pending = self._pending_automatic_input_items(prefetched)
        if not pending:
            return
        deadline_at = time.time() + self.turn_response_timeout
        while pending:
            remaining = deadline_at - time.time()
            if remaining <= 0:
                raise TimeoutError(
                    f"automatic VAD input items did not reach a terminal commit within "
                    f"{self.turn_response_timeout:.1f}s: {sorted(pending)}"
                )
            try:
            ...
            prefetched.append(received)
            if received.event["type"] == "error":
                return
            pending = self._pending_automatic_input_items(prefetched)

    @staticmethod
    def _latest_input_speech_boundary_at(prefetched: deque[_ReceivedEvent]) -> float | None:
        boundaries = [
            received.received_at for received in prefetched if received.event["type"] in _INPUT_SPEECH_BOUNDARY_EVENTS
        ]
    ...
    async def _continuous_turn_loop(self, websocket) -> None:
        ...
        while not self.shutdown_requested():
            ...
            metric_lengths = {name: len(samples) for name, samples in self.server_metric_samples.items()}
            response_status_counts = self.response_status_counts.copy()
            try:
                turn = await self._run_turn(websocket, turn_input)
            except (TimeoutError, RealtimeTurnError) as exc:
                self._restore_turn_metrics(metric_lengths, response_status_counts)
    ...
    def _restore_turn_metrics(
        self,
        metric_lengths: dict[str, int],
        response_status_counts: dict[str, int],
    ) -> None:
        """Remove samples recorded by a turn that did not complete."""
        for name in tuple(self.server_metric_samples):
            if name not in metric_lengths:
                del self.server_metric_samples[name]
                continue
            del self.server_metric_samples[name][metric_lengths[name] :]
        self.response_status_counts.clear()
        self.response_status_counts.update(response_status_counts)
    ...
    async def _run_turn(self, websocket, turn_input_value: Path | str) -> _TurnResult:
        ...
        if self.input_mode == "audio":
            ...
            if self.turn_mode == "automatic":
                input_finished_at, _ = await self._stream_pcm_with_event_prefetch(
                    websocket,
                    pcm,
                    prefetched_events,
                )
                turn_input.audio_source_end_ms = round(self._input_audio_bytes * 1000 / (2 * REALTIME_PCM_RATE))
            else:
            ...
            if self.turn_mode == "automatic":
                if self.vad_silence_ms:
                    ...
                    tail_finished_at, tail_boundary_observed = await self._stream_pcm_with_event_prefetch(
                        websocket,
                        silence,
                        prefetched_events,
                    )
                    if tail_boundary_observed:
                        input_finished_at = tail_finished_at
                await self._prefetch_until_automatic_input_items_are_closed(prefetched_events)
                latest_boundary_at = self._latest_input_speech_boundary_at(prefetched_events)
                if latest_boundary_at is not None:
                    input_finished_at = latest_boundary_at
            ...
            while True:
                ...
                if event_type == "response.created":
                    ...
                    if self.input_mode == "text" and not response_ids:
                        user_item = self._conversation_items.get(turn_input.text_item_id or "")
                    ...
                    self._bind_response_to_input_items(
                        response_id,
                        response_ids,
                        turn_tool_call_ids,
                        turn_input,
                ...
                if event_type in {
                    ...
                    if (
                        ...
                        and item.get("id") == turn_input.text_item_id
                ...
                if event_type == "response.done":
                    ...
                    if has_expected_output:
                        ...
                        for input_item_id in turn_input.response_item_ids[response_id]:
                            turn_input.items[input_item_id].final_response_id = response_id
                        if self._turn_input_complete(turn_input) and not any(
                            event.event["type"] in _INPUT_LIFECYCLE_EVENTS for event in prefetched_events
                        ):
                            return _TurnResult(
    ...
    async def _submit_text_turn(
        ...
        item_id = f"item_{uuid.uuid4().hex}"
        turn_input.items[item_id] = _TurnInputItemTrace(
            is_audio=False,
            submitted=True,
        )
        turn_input.submission_order.append(item_id)
        turn_input.text_item_id = item_id
    ...
    def _capture_item_event(
        ...
        if item_type == "message" and item.get("role") == "user" and turn_input.submitted_text is not None:
            ...
            if item_id != turn_input.text_item_id or item.get("content") != expected_content:
        ...
        if event_type == "conversation.item.added":
            ...
            client_text_item = bool(
                ...
                and turn_input.submitted_text is not None
                and turn_input.text_item_id == item_id
            )
            ...
            if item_type == "message" and item.get("role") == "user":
                input_item = turn_input.items.get(item_id)
                if input_item is None or not input_item.submitted:
                    raise RealtimeClientError(
                        f"conversation user item {item_id!r} does not match a submitted item for this turn"
                    )
    ...
    def _capture_input_lifecycle_event(self, event: dict[str, Any], turn_input: _TurnInputTrace) -> None:
        ...
        if event_type == "input_audio_buffer.speech_started":
            ...
            if self.turn_mode != "automatic":
                raise RealtimeClientError("Received speech_started while automatic turn detection is disabled")
            if item_id in turn_input.items:
                raise RealtimeClientError(f"input_audio_buffer.speech_started reused item ID {item_id!r}")
            prior_audio_items = [item for item in turn_input.items.values() if item.is_audio]
            if prior_audio_items:
                prior = prior_audio_items[-1]
                if prior.speech_end_ms is None:
                    raise RealtimeClientError(
                        "input_audio_buffer.speech_started arrived before the preceding speech boundary"
                    )
                if audio_start_ms < prior.speech_end_ms:
                    raise RealtimeClientError(
                        "input_audio_buffer.speech_started.audio_start_ms overlaps the preceding speech item"
                    )
                raise RealtimeTurnError(
                    "automatic VAD split one benchmark WAV into multiple input items; "
                    "use one continuous utterance per WAV with no long internal pauses "
                    "(or manual turn mode for intentional pauses)"
                )
            turn_input.items[item_id] = _TurnInputItemTrace(
                is_audio=True,
                speech_start_ms=audio_start_ms,
            )
            return
        if event_type in {"input_audio_buffer.speech_stopped", "input_audio_buffer.timeout_triggered"}:
            ...
            input_item = turn_input.items.get(item_id)
            if input_item is None or not input_item.is_audio or input_item.speech_start_ms is None:
                raise RealtimeClientError(f"{event_type} does not match speech_started for this turn")
            if audio_end_ms < input_item.speech_start_ms:
                raise RealtimeClientError(f"{event_type}.audio_end_ms precedes speech_started.audio_start_ms")
            if turn_input.audio_source_end_ms is not None and audio_end_ms < turn_input.audio_source_end_ms:
                raise RealtimeTurnError(
                    "automatic VAD ended a benchmark input item before the WAV finished; "
                    "use one continuous utterance per WAV with no long internal or trailing pauses "
                    "(or manual turn mode for intentional pauses)"
                )
            if event_type == "input_audio_buffer.timeout_triggered":
                ...
                if timeout_start != input_item.speech_start_ms:
                    raise RealtimeClientError("input_audio_buffer.timeout_triggered changed audio_start_ms")
            if input_item.speech_boundary_event is not None:
                raise RealtimeClientError(f"{event_type} repeated the terminal speech boundary")
            input_item.speech_end_ms = audio_end_ms
            input_item.speech_boundary_event = event_type
            return
        if event_type == "input_audio_buffer.committed":
            item_id = _require_nonempty_string(event.get("item_id"), f"{event_type}.item_id")
            ...
            input_item = turn_input.items.get(item_id)
            if self.turn_mode == "automatic":
                if input_item is None or input_item.speech_boundary_event is None:
                    raise RealtimeClientError(
                        "input_audio_buffer.committed must follow speech_started/stopped for the same item"
                    )
            else:
                if turn_input.submission_order:
                    raise RealtimeClientError("Received more than one input_audio_buffer.committed for a manual turn")
                if input_item is not None:
                    raise RealtimeClientError(f"input_audio_buffer.committed reused item ID {item_id!r}")
                input_item = _TurnInputItemTrace(is_audio=True)
                turn_input.items[item_id] = input_item
            assert input_item is not None
            if input_item.submitted:
                raise RealtimeClientError(f"input_audio_buffer.committed repeated item ID {item_id!r}")
            input_item.submitted = True
            turn_input.submission_order.append(item_id)
            return
    ...
    def _bind_response_to_input_items(
        self,
        response_id: str,
        response_ids: list[str],
        turn_tool_call_ids: set[str],
        ...
        """Bind a response snapshot to its submitted input items."""
        ...
        if response_ids:
            ...
            if previous.has_function_call or previous.has_mcp_call:
                ...
                pending_mcp_items = [
                    output.item_id
                    for output in self._response_mcp_outputs(previous.response_id)
                    if not output.output_done
                ]
                if pending_mcp_items:
                    raise RealtimeClientError(
                        f"response.created started {response_id!r} before terminal MCP items "
                        f"{sorted(pending_mcp_items)}"
                    )
                input_item_ids = turn_input.response_item_ids[previous.response_id]
            else:
                input_item_ids = tuple(
                    item_id for item_id in turn_input.submission_order if not turn_input.items[item_id].response_ids
                )
        else:
            input_item_ids = tuple(turn_input.submission_order)
        ...
        for item_id in input_item_ids:
            item = self._conversation_items.get(item_id)
            if item is None or item.done is None:
                raise RealtimeClientError(
                    f"response.created started before submitted conversation item {item_id!r} was terminal"
                )
            turn_input.items[item_id].response_ids.append(response_id)
        turn_input.response_item_ids[response_id] = input_item_ids

    @staticmethod
    def _turn_input_complete(turn_input: _TurnInputTrace) -> bool:
        """Return whether every observed input item has one final response."""
        if not turn_input.submission_order:
            return False
        if any(item.is_audio and not item.submitted for item in turn_input.items.values()):
            return False
        return all(turn_input.items[item_id].final_response_id is not None for item_id in turn_input.submission_order)
